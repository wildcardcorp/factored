    ...
    def handle(self, host, settings, params, datastore, finder):
        ...
        if submit == "email" and subject is not None:
            if not finder.is_valid_subject(host, subject):
                ...
                try:
                    self.generate_and_send_code(
                        host, settings, params, datastore, subject)
        ...
        # must confirm code and generate jwt or reject
            ...
            if not finder.is_valid_subject(host, subject):
                ...
                try:
                    auditlog.info(subject)
                    auditlog.info(repr(host))
                    auditlog.info(repr(settings))
                    auditlog.info(repr(params))
                    auditlog.info(repr(datastore))
                    auditlog.info(repr(finder))
                    self.validate_code(host, settings, params,
                                       datastore, subject, code)
                    _host, _subject, _timestamp, _payload = datastore.get_access_request(host, subject)
                    return {
                        "authenticated": True,
                        "subject": subject,
                        "extra_info": json.dumps({
                            '_host': repr(_host),
                            '_subject': repr(_subject),
                            '_timestamp': repr(_timestamp),
